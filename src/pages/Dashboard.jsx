...
function ChartTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: '#222', border: '1px solid #444', borderRadius: 6, padding: '8px 12px', fontSize: 13 }}>
      <div style={{ color: '#ccc', fontWeight: 600 }}>{d.state || d.name || d.bracket}</div>
...
function NotableDonorGroup({ group, allExpanded }) {
  const [expanded, setExpanded] = useState(false)
  const show = allExpanded || expanded
  const visible = show ? group.donors : group.donors.slice(0, 4)
  ...
  return (
    <div className="wrap">
      ...
      <div className="cards">
        ...
        <StatCard label="AIPAC conduit total" value={fmtFull(s.aipacTotal)} sub={`${s.aipacTransactions} earmarked contributions`} warn />
      </div>
      {/* Stat row 2 */}
      <div className="cards mb">
        <StatCard label="PAC / committee money" value={fmtFull(s.pacMoney)} sub={`${s.pacPercent}% of all receipts`} />
        {s.candidateSelfFunding > 0 && (
          <StatCard label="Candidate self-funding" value={fmtFull(s.candidateSelfFunding)} sub={s.selfFundingNote} />
        )}
        <StatCard label="Individual donations" value={fmtFull(s.individualTotal)} sub={`${s.individualTransactions} positive transactions`} />
        <StatCard label="Avg individual donation" value={`$${s.avgDonation.toLocaleString()}`} sub={`Median: $${s.medianDonation.toLocaleString()} · ${s.maxedDonations} maxed at $3,500`} />
        {s.aipacPercent >= 5 && (
          <StatCard label="AIPAC as % of individual $" value={`${s.aipacPercent}%`} sub={s.aipacBundled > 0 ? `$${(s.aipacBundled/1000).toFixed(0)}K bundled through AIPAC` : ''} warn />
        )}
      </div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data.stateBreakdown} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
      ...
      {/* TOP PACS CHART */}
      <div className="divider" />
      <h2>Top PAC / organization donors</h2>
      {data.summary.aipacBundled > 0 && (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 16 }}>
          AIPAC total includes ${(data.summary.aipacBundled/1000).toFixed(0)}K in bundled earmarked contributions + direct PAC + DMFI PAC
        </div>
      )}
      ...
      <div className="insight">
        <h3 className="warn">AIPAC + Israel-aligned total: {fmtFull(data.summary.israelAlignedTotal)}</h3>
      ...
      <div className="big-stats">
        ...
        <div className="big-stat"><div className="num">{s.maxedDonations}</div><div className="desc">donations hit the $3,500 federal maximum</div></div>
