...
export default function Home() {
  ...
  const [sortBy, setSortBy] = useState('aipacTotal-desc')
  ...
  const filtered = politicians
    ...
    .sort((a, b) => {
      ...
      let cmp = 0
      if (field === 'aipacTotal') cmp = a.aipacTotal - b.aipacTotal
      else if (field === 'outOfState') cmp = a.outOfStatePercent - b.outOfStatePercent
      else if (field === 'totalRaised') cmp = a.totalRaised - b.totalRaised
      else if (field === 'avgDonation') cmp = (a.avgDonation || 0) - (b.avgDonation || 0)
      else if (field === 'inState') cmp = a.homeStatePercent - b.homeStatePercent
      else if (field === 'pacPercent') cmp = (a.pacPercent || 0) - (b.pacPercent || 0)
  ...
  return (
    <div className="wrap">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        >
          <optgroup label="Pro-Israel Lobby $">
            <option value="aipacTotal-desc">Pro-Israel Lobby $ (high → low)</option>
            <option value="aipacTotal-asc">Pro-Israel Lobby $ (low → high)</option>
          </optgroup>
          <optgroup label="Out-of-State %">
            <option value="outOfState-desc">Out-of-state % (high → low)</option>
            <option value="outOfState-asc">Out-of-state % (low → high)</option>
          </optgroup>
          <optgroup label="In-State %">
            <option value="inState-desc">In-state % (high → low)</option>
            <option value="inState-asc">In-state % (low → high)</option>
          </optgroup>
          <optgroup label="Avg Donation">
            <option value="avgDonation-desc">Avg donation (high → low)</option>
            <option value="avgDonation-asc">Avg donation (low → high)</option>
          </optgroup>
          <optgroup label="Total Raised">
            <option value="totalRaised-desc">Total raised (high → low)</option>
            <option value="totalRaised-asc">Total raised (low → high)</option>
          </optgroup>
          <optgroup label="PAC %">
            <option value="pacPercent-desc">PAC % (high → low)</option>
            <option value="pacPercent-asc">PAC % (low → high)</option>
          </optgroup>
          <optgroup label="Name">
